...
let get_indice_min_array_init array sommet_marque =
...
let rec traitement_point graphe tab_d tab_p tab_m =
    if (Array.length tab_m == Array.length tab_d)
    then (tab_d, tab_p)
    else (
...
let dijkstra_tas graphe indice_depart =
    [| 0. |], [| 0 |];;

print_string "Dijkstra avec un tas : \n";;
let arrayD, arrayP = dijkstra_tas grapheComplet 0 ;;
print_string("tableau des distance = [") ;; Array.iter (printf "%.0f. ") arrayD;; print_string("]\n");;
print_string("tableau des prédécesseurs = [") ;; Array.iter (printf "%d ") arrayP;; print_string("]\n");;
*)
...
let appartient_cerle coords =
    ((fst coords)*(fst coords)) + ((snd coords)*(snd coords)) == 100;;

let appartient_carre coords =
    (fst coords) > 100 && (fst coords) < 300 && (snd coords) > 0 && (snd coords) < 500;;

(*print_string "Liste de sommet : [";; Array.iter (fun s -> printf "(%d, %d), " (fst s) (snd s)) sommets;; print_string "]\n";;*)
...
let sommets = genere_sommets 1000 [appartient_cerle; appartient_carre];;
affichage_sommets sommets;;
(*
