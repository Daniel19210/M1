...
let dijkstra_tas graphe sommet_depart =
    ...
    let rec dijkstra_aux tas_sommets_restants =
        match tas_sommets_restants with
        | Vide -> distances, predecesseurs
        | Noeud (fils_gauche, sommet_dist_min, fils_droit) -> (
            ...
            dijkstra_aux (supprimer_premier_noeud tas_sommets_restants)
        )
    in dijkstra_aux (creer_tas (sequence 0 nb_sommets)) ;;

creer_tas (sequence 0 10);;

let arrayD, arrayP = dijkstra_tas grapheComplet 0 ;;
print_string("\nDijkstra solution du professeur avec tas manuel\n");;
print_string("tableau des distance = [ ") ;; Array.iter (printf "%.0f. ") arrayD;; print_string("]\n");;
print_string("tableau des prédécesseurs = [ ") ;; Array.iter (printf "%d ") arrayP;; print_string("]\n");;

let dijkstra_prof graphe sommet_depart = 
    let nb_sommets = Array.length graphe.sommets in
    let distances = Array.make nb_sommets infinity in
    let predecesseurs = Array.make nb_sommets (-1) in
...
let arrayD, arrayP = dijkstra_prof grapheComplet 0 ;;
print_string("\nDijkstra solution du professeur\n");;
print_string("tableau des distance = [ ") ;; Array.iter (printf "%.0f. ") arrayD;; print_string("]\n");;
print_string("tableau des prédécesseurs = [ ") ;; Array.iter (printf "%d ") arrayP;; print_string("]\n");;
...
let arrayD, arrayP = dijkstra_prof grapheComplet 0 ;;
print_string("\nDijkstra solution du professeur avec tas\n");;
print_string("tableau des distance = [ ") ;; Array.iter (printf "%.0f. ") arrayD;; print_string("]\n");;
print_string("tableau des prédécesseurs = [ ") ;; Array.iter (printf "%d ") arrayP;; print_string("]\n");;
