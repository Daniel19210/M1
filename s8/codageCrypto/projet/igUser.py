...
def dechiffrageMessage(inputtxt, outputtxt):
    messageDechiffre = m.dechiffrage(m.lectureFichier(
        "cle.txt"), m.formattageMessage(inputtxt.get("1.0", 'end-1c')))
    outputtxt.delete("1.0", END)
...
def initializeFenetre():
    ...
    label4 = Label(fenetre, text="Clé de chiffrement")
    ...
    cryptButton = Button(fenetre, text="Chiffrer Message", command=lambda: chiffrageMessage(
        inputtxt, outputtxt, resultatMessage, affichCle))
    decryptButton = Button(fenetre, text="Déchiffrer Message",
                           command=lambda: dechiffrageMessage(outputtxt, resultatMessage))
    setCleDechiffButton = Button(
        fenetre, text="Insérer cle de Dechiffrage", command=lambda: setCleDechiff())
    openFileACoderButton = Button(
        fenetre, text="Insérer message a Coder", command=lambda: ouvrirFichierCode(inputtxt))
    openFileADecoderButton = Button(
        fenetre, text="Insérer message a Décoder", command=lambda: ouvrirFichierDecode(outputtxt))

    copyPressePapierButton = Button(
        fenetre, text="Copier la clé dans le presse-papiela clé dans le presse-papier", command=lambda: copieCle())
    ...
    copyPressePapierButton.grid(row=2, column=3, padx=10, pady=10, sticky="ns")
